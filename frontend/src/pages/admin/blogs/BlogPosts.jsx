...
import { BlogLoader, PostFormDrawer } from "../../../components";
  ...
  const onDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        toast.promise(dispatch(deleteBlog(id)), {
          loading: "Deleting...",
      ...
      <div className="flex gap-6 flex-col">
        ...
        <div role="tablist" className="tabs tabs-border">
          {statusData.map((s, i) => (
            ...
            >
              <span> {s.label}</span>
              <span className="size-5 text-sm flex items-center justify-center rounded-full bg-accent">
                {s.count}
              </span>
            </div>
          ))}
